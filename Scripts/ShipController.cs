...
public class ShipController : MonoBehaviour
{
    ...
    private float trailLingerTime = 0.5f; // Time to keep trails active after stopping
    private float normalTrailTimer = 0f;  // Timer for normal trail linger
    private float boostTrailTimer = 0f;   // Timer for boost trail linger
    ...
    void Start()
    {
        ...
        {
            normalTrail.Stop();
            Debug.Log("Normal trail initialized.");
        }
        if (boostTrail != null)
        {
            boostTrail.Stop();
            Debug.Log("Boost trail initialized.");
        }
    }
    ...
    void HandleMovement()
    {
        ...
        bool isBoosting = Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.W);
        currentSpeed = isBoosting ? moveSpeed * boostSpeedMultiplier : moveSpeed;

        if (Input.GetKey(KeyCode.W))
        {
            movement += transform.forward * currentSpeed;
            Debug.Log($"W pressed, moving forward at speed {currentSpeed} (Boosting: {isBoosting})");
        }
    ...
    void HandleParticleTrails()
    {
        bool isMovingForward = Input.GetKey(KeyCode.W);
        bool isBoosting = Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.W);

        // Normal trail: active when moving forward, but not boosting
        if (normalTrail != null)
        {
            if (isMovingForward && !isBoosting)
            {
                if (!normalTrail.isPlaying) normalTrail.Play();
                normalTrailTimer = trailLingerTime; // Reset linger timer
            }
            else
            {
                normalTrailTimer -= Time.deltaTime;
                if (normalTrailTimer <= 0 && normalTrail.isPlaying)
                {
                    normalTrail.Stop();
                }
            }
        ...
        // Boost trail: active only when boosting
        if (boostTrail != null)
        {
            if (isBoosting)
            {
                if (!boostTrail.isPlaying) boostTrail.Play();
                boostTrailTimer = trailLingerTime; // Reset linger timer
            }
            else
            {
                boostTrailTimer -= Time.deltaTime;
                if (boostTrailTimer <= 0 && boostTrail.isPlaying)
                {
                    boostTrail.Stop();
                }
            }
